 ...
 * Gemini AI Driver
 *
 * 透過 Google Gemini REST API 實作向量嵌入、文字生成與翻譯功能。
 * 支援 text-embedding-004（嵌入）與 gemini-2.5-flash（生成/翻譯）模型。
 * API 金鑰以 query parameter 方式傳遞。
 */
class GeminiDriver implements EmbeddingInterface, LlmInterface, TranslatorInterface
{
    /**
     * Driver 設定
     *
     ...
     * 將單一或多段文字轉換為向量嵌入
     *
     * 使用 Gemini embedContent API 將文字轉換為向量表示。
     * 當傳入陣列時，會取第一個元素進行嵌入。
     *
    ...
    public function embed(string|array $text): array
    {
        ...
        $url = "{$this->baseUrl}/models/{$embeddingModel}:embedContent";
     ...
     * 批次將多段文字轉換為向量嵌入
     *
     * 使用 Gemini batchEmbedContents API 一次處理多段文字。
     *
    ...
    public function batchEmbed(array $texts): array
    {
        ...
        $url = "{$this->baseUrl}/models/{$embeddingModel}:batchEmbedContents";
        ...
        return array_map(
            fn (array $embedding): array => $embedding['values'] ?? [],
            $embeddings,
        );
    }
     ...
     * 根據提示詞生成文字回應
     *
     * 使用 Gemini generateContent API 產生文字內容。
     *
     ...
     *
     * 使用 Gemini generateContent API 搭配 response_mime_type 與 response_schema
    ...
    public function generateStructured(string $prompt, array $schema, array $options = []): array
    {
        ...
        if (! is_array($decoded)) {
            throw new RuntimeException(
                "Gemini API 結構化輸出 JSON 解析失敗：{$text}"
            );
        }
...
PROMPT;
     ...
     * 解析嵌入模型名稱
     *
     * 若設定中的模型為嵌入模型（如 text-embedding-004），直接使用；
     * 否則使用預設嵌入模型。
     *
    ...
    /**
     * 建構 generationConfig 參數
     *
     * 將 options 中的參數映射為 Gemini API 的 generationConfig 格式。
     * max_tokens 會映射為 maxOutputTokens。
     *
     * @param  array<string, mixed>  $options  選項參數
     * @return array<string, mixed>  generationConfig 陣列
     */
...
PROMPT;
    ...
    protected function sendRequest(string $url, array $payload): array
    {
        ...
        if ($response->failed()) {
            ...
            throw new RuntimeException(
                "Gemini API 請求失敗（HTTP {$status}）：{$body}"
            );
        }
        ...
        if (isset($data['error'])) {
            ...
            throw new RuntimeException(
                "Gemini API 錯誤（{$errorCode}）：{$errorMessage}"
            );
        }
